...
export async function POST(req: Request) {
  try {
    ...
    const prompt = `
다음은 한 명 또는 여러 명의 고객 정보가 담긴 보험 청약서 혹은 명단 문서의 스캔 텍스트입니다. 이 텍스트에서 모든 고객 정보를 추출하여 JSON 형식의 배열로 반환해 주세요.
반드시 아래 스키마에 맞는 JSON 텍스트만 출력해야 하며, 설명이나 다른 텍스트는 포함하지 마세요. 문서에 고객이 1명이든 20명이든 모두 추출해야 합니다.
